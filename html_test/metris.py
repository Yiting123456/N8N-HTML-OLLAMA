...
def get_tag_values(tag_id: int) -> dict:
    ...
    try:
        ...
        resp_info = ""
        try:
            resp_info = f" response: {resp.status_code} {resp.text}"
        except Exception:
            pass
    ...
    try:
        ...
        raise MetrisResponseError(f"tagvalues 返回非 JSON: {resp.text}")
    ...
    for tag_id in ids:
        try:
            ...
            if response.status_code == 200:
                ...
                result[tag_id] = {'error': f"Failed to retrieve data for tag_id {tag_id} (status {response.status_code})"}
        except Exception as e:
            result[tag_id] = {'error': str(e)}
    return result